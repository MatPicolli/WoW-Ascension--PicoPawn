...
function PawnInitialize()
	...
	for ScaleName, _ in pairs(PicoPawnCommon.Scales) do
...
-- called by any code that might run before initialization finishes to ensure that PicoPawnCommon exists and is set up.
function PawnInitializeOptions()
	-- If either of the options tables don't exist yet, create them now.
	if not PicoPawnCommon then PicoPawnCommon = {} end
	if not PicoPawnOptions then PicoPawnOptions = {} end
	
	-- We need to know the player's full name for some server-specific settings.
	PawnPlayerFullName = UnitName("player") .. "-" .. GetRealmName()
	-- Save the last known player name to PicoPawnOptions so that we can detect character renames and server
	-- transfers in the future.
	PicoPawnOptions.LastPlayerFullName = PawnPlayerFullName
	
	-- Now, migrate all settings over to PicoPawnCommon, and upgrade to the current version from any previous version
	-- of Pawn (or none at all).  Settings are respected in this order of preference:
	-- 1. Global settings in PicoPawnCommon
	-- 2. Per-character settings in PicoPawnOptions (used prior to Pawn 1.3)
	...
	if PicoPawnOptions.Scales then
		-- Looks like there's one or more scales on this character that need to be migrated.
		for ScaleName, Scale in pairs(PicoPawnOptions.Scales) do
			if PicoPawnCommon.Scales[ScaleName] then
				...
				local i = 0
				while PicoPawnCommon.Scales[ScaleName] do
					i = i + 1
			...
			PicoPawnCommon.Scales[ScaleName] = Scale
			...
			Scale.NormalizationFactor = PicoPawnOptions.NormalizationFactor
			Scale.Hidden = nil
		end
	end
	-- Now that migration is complete, remove all migrated scales from the per-character options.
	PicoPawnOptions.Scales = nil
	
	-- These options have been removed or otherwise are no longer useful.
	PicoPawnOptions.ShowItemLevel = nil
	PicoPawnOptions.ShownGettingStarted = nil
	PicoPawnOptions.NormalizationFactor = nil
...
function PawnMigrateSetting(SettingName, Default)
	if PicoPawnCommon[SettingName] ~= nil then
		PicoPawnOptions[SettingName] = nil
		return
	end
	if PicoPawnOptions[SettingName] ~= nil then
		PicoPawnCommon[SettingName] = PicoPawnOptions[SettingName]
		PicoPawnOptions[SettingName] = nil
		return
	end
	PicoPawnCommon[SettingName] = Default
end
...
function PawnSetDefaultKeybindings()
	...
	if PicoPawnOptions.LastKeybindingsSet == nil  then PicoPawnOptions.LastKeybindingsSet = 0 end
	local BindingSet = false
	
	-- Keybindings for opening the Pawn UI and setting comparison items.
	if PicoPawnOptions.LastKeybindingsSet < 1 then
	...
	PicoPawnOptions.LastKeybindingsSet = 1
end
...
function PawnCommand(Command)
	...
	elseif Command == PawnLocal.DebugOnCommand then
		PicoPawnCommon.Debug = true
		PawnResetTooltips()
		if PawnUIFrame_DebugCheck then PawnUIFrame_DebugCheck:SetChecked(PicoPawnCommon.Debug) end
	elseif Command == PawnLocal.DebugOffCommand then
		PicoPawnCommon.Debug = false
		PawnResetTooltips()
		if PawnUIFrame_DebugCheck then PawnUIFrame_DebugCheck:SetChecked(PicoPawnCommon.Debug) end
...
function PawnGetCachedItem(ItemLink, ItemName, NumLines)
	...
	if PicoPawnCommon.Debug then return end
...
function PawnRecalculateScaleTotal(ScaleName)
	...
	local ThisScale = PicoPawnCommon.Scales[ScaleName]
...
function PawnRecreateAnnotationFormats()
	PawnUnenchantedAnnotationFormat = "%s%s:  %." .. PicoPawnCommon.Digits .. "f"
	PawnEnchantedAnnotationFormat = PawnUnenchantedAnnotationFormat .. "  %s(%." .. PicoPawnCommon.Digits .. "f " .. PawnLocal.BaseValueWord .. ")"
end
...
function PawnUpdateTooltip(TooltipName, MethodName, Param1, ...)
	if not PicoPawnCommon.Scales then return end
	...
	local AddSpace = PicoPawnCommon.ShowSpace
	...
	if Item.UnknownLines and (PicoPawnCommon.ShowAsterisks == PawnShowAsterisksAlways) or ((PicoPawnCommon.ShowAsterisks == PawnShowAsterisksNonzero or PicoPawnCommon.ShowAsterisks == PawnShowAsterisksNonzeroNoText) and #Item.Values > 0) then
	...
	if (Annotated and PicoPawnCommon.ShowAsterisks ~= PawnShowAsterisksNonzeroNoText) then
	...
	if PicoPawnCommon.ShowItemID and Item.Link then
		local IDs = PawnGetItemIDsForDisplay(Item.Link)
		if IDs then
			if PicoPawnCommon.AlignNumbersRight then
...
function PawnGetAllItemValues(Item, SocketBonus, UnenchantedItem, UnenchantedItemSocketBonus, DebugMessages, NoNormalization)
	local ItemValues = {}
	for ScaleName, Scale in pairs(PicoPawnCommon.Scales) do
		...
		if ShowScale then -- Skip all disabled scales.  PawnGetSingleValueFromItem will calculate them on-demand if necessary.
			...
			if UnenchantedItem then
				UnenchantedValue, UseRed, UseYellow, UseBlue = PawnGetItemValue(UnenchantedItem, UnenchantedItemSocketBonus, ScaleName, ShowScale and DebugMessages and PicoPawnCommon.ShowUnenchanted, NoNormalization)
			end
			if Item then
				if ShowScale and DebugMessages and PicoPawnCommon.ShowEnchanted and PicoPawnCommon.ShowUnenchanted then
					PawnDebugMessage(" ")
					PawnDebugMessage(PawnLocal.EnchantedStatsHeader)
				end
				Value = PawnGetItemValue(Item, SocketBonus, ScaleName, ShowScale and DebugMessages and PicoPawnCommon.ShowEnchanted, NoNormalization)
			end
...
function PawnAddValuesToTooltip(Tooltip, ItemValues, OnlyFirstValue)
	...
	for _, Entry in pairs(ItemValues) do
		...
		local Scale = PicoPawnCommon.Scales[ScaleName]
		...
		if PawnIsScaleVisible(ScaleName) then
			...
			if OnlyFirstValue then
				...
				if not PicoPawnCommon.ShowEnchanted then Value = 0 end
				if not PicoPawnCommon.ShowUnenchanted then UnenchantedValue = 0 end
			end
			...
			if Value and Value > 0 and UnenchantedValue and UnenchantedValue > 0 and math.abs(Value - UnenchantedValue) >= ((10 ^ -PicoPawnCommon.Digits) / 2) then
...
function PawnGetItemValue(Item, SocketBonus, ScaleName, DebugMessages, NoNormalization)
	...
	local ScaleOptions = PicoPawnCommon.Scales[ScaleName]
...
function PawnCorrectScaleErrors(ScaleName)
	local ThisScaleOptions = PicoPawnCommon.Scales[ScaleName]
...
function PawnFindBestGems(ScaleName, FindRed, FindYellow, FindBlue, FindMeta)
	...
	if FindMeta then
		GemTable = PawnMetaGemQualityTables[PicoPawnCommon.Scales[ScaleName].MetaGemQualityLevel]
	else
		GemTable = PawnGemQualityTables[PicoPawnCommon.Scales[ScaleName].GemQualityLevel]
	end
...
function PawnResetScalesCore(ResetCustomScales, ResetProviderScales)
	...
	for _, ScaleName in pairs(ScalesToRemove) do
		PicoPawnCommon.Scales[ScaleName] = nil
	end
	PawnResetTooltips()
	return true
end
...
function PawnAddEmptyScale(ScaleName)
	if (not ScaleName) or (ScaleName == "") then
		...
		return false
	elseif PicoPawnCommon.Scales[ScaleName] then
		VgerCore.Fail("ScaleName cannot be the name of an existing scale, and is case-sensitive.")
		return false
	end
	
	PicoPawnCommon.Scales[ScaleName] = PawnGetEmptyScale()
	PicoPawnCommon.Scales[ScaleName].PerCharacterOptions[PawnPlayerFullName] = { }
	PicoPawnCommon.Scales[ScaleName].PerCharacterOptions[PawnPlayerFullName].Visible = true
	PawnRecalculateScaleTotal(ScaleName)
	return true
end
...
function PawnAddDefaultScale(ScaleName)
	...
	PicoPawnCommon.Scales[ScaleName] = PawnGetDefaultScale()
	PicoPawnCommon.Scales[ScaleName].PerCharacterOptions[PawnPlayerFullName] = { }
	PicoPawnCommon.Scales[ScaleName].PerCharacterOptions[PawnPlayerFullName].Visible = true
	PawnRecalculateScaleTotal(ScaleName)
	PawnResetTooltips()
...
function PawnDeleteScale(ScaleName)
	if (not ScaleName) or (ScaleName == "") then
		...
		return false
	elseif not PicoPawnCommon.Scales[ScaleName] then
...
function PawnRenameScale(OldScaleName, NewScaleName)
	...
	elseif OldScaleName == NewScaleName then
		...
		return false
	elseif not PicoPawnCommon.Scales[OldScaleName] then
		VgerCore.Fail("OldScaleName must be the name of an existing scale, and is case-sensitive.")
		return false
	elseif PicoPawnCommon.Scales[NewScaleName] then
	...
	PicoPawnCommon.Scales[NewScaleName] = PicoPawnCommon.Scales[OldScaleName]
	PicoPawnCommon.Scales[OldScaleName] = nil
...
function PawnDuplicateScale(OldScaleName, NewScaleName)
	...
	-- Create the copy.
	PicoPawnCommon.Scales[NewScaleName] = {}
	PicoPawnCommon.Scales[NewScaleName].Color = PicoPawnCommon.Scales[OldScaleName].Color
	PicoPawnCommon.Scales[NewScaleName].SmartGemSocketing = PicoPawnCommon.Scales[OldScaleName].SmartGemSocketing
	PicoPawnCommon.Scales[NewScaleName].GemQualityLevel = PicoPawnCommon.Scales[OldScaleName].GemQualityLevel
	PicoPawnCommon.Scales[NewScaleName].SmartMetaGemSocketing = PicoPawnCommon.Scales[OldScaleName].SmartMetaGemSocketing
	PicoPawnCommon.Scales[NewScaleName].MetaGemQualityLevel = PicoPawnCommon.Scales[OldScaleName].MetaGemQualityLevel
	PicoPawnCommon.Scales[NewScaleName].NormalizationFactor = PicoPawnCommon.Scales[OldScaleName].NormalizationFactor
	PicoPawnCommon.Scales[NewScaleName].PerCharacterOptions = {}
	PicoPawnCommon.Scales[NewScaleName].PerCharacterOptions[PawnPlayerFullName] = {}
	PicoPawnCommon.Scales[NewScaleName].PerCharacterOptions[PawnPlayerFullName].Visible = true
	PicoPawnCommon.Scales[NewScaleName].Values = {}
	local NewScale = PicoPawnCommon.Scales[NewScaleName].Values
	for StatName, Value in pairs(PicoPawnCommon.Scales[OldScaleName].Values) do
...
function PawnGetStatValue(ScaleName, StatName)
	...
	return PicoPawnCommon.Scales[ScaleName].Values[StatName]
end
...
function PawnGetAllStatValues(ScaleName)
	...
	--local TableCopy = {}
	--for StatName, Value in pairs(PicoPawnCommon.Scales[ScaleName].Values) do
	--	TableCopy[StatName] = Value
	--end
	--return TableCopy
	return PicoPawnCommon.Scales[ScaleName].Values
end
...
function PawnSetStatValue(ScaleName, StatName, Value)
	...
	PicoPawnCommon.Scales[ScaleName].Values[StatName] = Value
...
function PawnGetGemQualityLevel(ScaleName)
	...
	return PicoPawnCommon.Scales[ScaleName].GemQualityLevel
end
...
function PawnSetGemQualityLevel(ScaleName, QualityLevel)
	...
	PicoPawnCommon.Scales[ScaleName].GemQualityLevel = QualityLevel
	PawnRecalculateScaleTotal(ScaleName) -- also recalculates socket values
	PawnResetTooltips()
	return true
end
...
function PawnGetScaleTag(ScaleName)
	...
	elseif not PicoPawnCommon.Scales[ScaleName] then
		VgerCore.Fail("ScaleName must be the name of an existing scale, and is case-sensitive.")
		return
	elseif not PicoPawnCommon.Scales[ScaleName].Values then
		return
	end
	...
	local IncludeThis
	local SmartGemSocketing = PicoPawnCommon.Scales[ScaleName].SmartGemSocketing
	local SmartMetaGemSocketing = PicoPawnCommon.Scales[ScaleName].SmartMetaGemSocketing
	for StatName, Value in pairs(PicoPawnCommon.Scales[ScaleName].Values) do
	...
	if AddComma then ScaleTag = ScaleTag .. ", " end
	ScaleTag = ScaleTag .. "GemQualityLevel=" .. tostring(PicoPawnCommon.Scales[ScaleName].GemQualityLevel)
...
function PawnImportScale(ScaleTag, Overwrite)
	...
	local AlreadyExists = PicoPawnCommon.Scales[ScaleName] ~= nil
	...
	if not AlreadyExists then
		...
		PicoPawnCommon.Scales[ScaleName] = { }
		PicoPawnCommon.Scales[ScaleName].PerCharacterOptions = { }
		PicoPawnCommon.Scales[ScaleName].PerCharacterOptions[PawnPlayerFullName] = { }
		PicoPawnCommon.Scales[ScaleName].PerCharacterOptions[PawnPlayerFullName].Visible = true
	end
	PicoPawnCommon.Scales[ScaleName].Values = Values	
	PawnCorrectScaleErrors(ScaleName)
	...
	if Values.GemQualityLevel then
		PicoPawnCommon.Scales[ScaleName].GemQualityLevel = Values.GemQualityLevel
		Values.GemQualityLevel = nil
	end
	if Values.MetaGemQualityLevel then
		PicoPawnCommon.Scales[ScaleName].MetaGemQualityLevel = Values.MetaGemQualityLevel
	...
	if not AlreadyExists then
		if (not Values.RedSocket) and (not Values.YellowSocket) and (not Values.BlueSocket) then
			PicoPawnCommon.Scales[ScaleName].SmartGemSocketing = true
		end
		if (not Values.MetaSocket) then
			PicoPawnCommon.Scales[ScaleName].SmartMetaGemSocketing = true
		end
	end
...
function PawnAddPluginScale(ProviderInternalName, ScaleInternalName, LocalizedName, Color, Values, NormalizationFactor)
	...
	local NewScale
	if PicoPawnCommon.Scales[ScaleFullName] then
		NewScale = PicoPawnCommon.Scales[ScaleFullName]
	else
	...
	if not PicoPawnCommon.Scales[ScaleFullName] then PicoPawnCommon.Scales[ScaleFullName] = NewScale end
